...
int main(int argc, char** argv) {	
	...
	switch (optresult){
		...
		case '?':
		default:
		fprintf(stderr, "Error: correct usage is ./lab4b_tcp [--period=<number>] [--scale=<number>] [--log=<filename>] [--host=<hostname>] [--id=<user_id>]\n");
		exit(1);
	}
	}
	...
	while(1){
		...
		if (pollInput.revents & POLLIN){ //if input is received, parse input
			int r;
			if ((r = read(sock_num, inputBuffer+buffercount, 64)) < 0){ //read input
